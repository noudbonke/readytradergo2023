                                         ...
                                         const std::array<unsigned long, TOP_LEVEL_COUNT>& bidVolumes)
{
    ...
    if (instrument == Instrument::FUTURE)
    {
        ...
        if (mAskId == 0 && newAskPrice != 0 && mPosition >= -POSITION_LIMIT + 3*LOT_SIZE)
        {
            mAskId = mNextMessageId++;
            mAskPrice = newAskPrice;
            SendInsertOrder(mAskId, Side::SELL, newAskPrice, 3*LOT_SIZE, Lifespan::GOOD_FOR_DAY);
            mAsks.emplace(mAskId);
        }
        if (mBidId == 0 && newBidPrice != 0 && mPosition <= POSITION_LIMIT-3*LOT_SIZE)
        {
            mBidId = mNextMessageId++;
            mBidPrice = newBidPrice;
            SendInsertOrder(mBidId, Side::BUY, newBidPrice, 3*LOT_SIZE, Lifespan::GOOD_FOR_DAY);
