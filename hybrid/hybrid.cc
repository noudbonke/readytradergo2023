                                         ...
                                         const std::array<unsigned long, TOP_LEVEL_COUNT>& bidVolumes)
{
    ...
    if (instrument == Instrument::ETF){
        ...
        int bidDiff = (futureLastBid-askPrices[0])/TICK_SIZE_IN_CENTS;
        int askDiff = (bidPrices[0]-futureLastAsk)/TICK_SIZE_IN_CENTS;
        ...
        if (mAskId == 0 && arbitrageAskPrice != 0 && mPosition > -POSITION_LIMIT)
        {
            ...
            SendInsertOrder(mAskId, Side::SELL, arbitrageAskPrice, std::min((signed int)(LOT_SIZE*askDiff), POSITION_LIMIT + (int)mPosition), Lifespan::GOOD_FOR_DAY);
        ...
        if (mBidId == 0 && arbitrageBidPrice != 0 && mPosition < POSITION_LIMIT)
        {
            ...
            SendInsertOrder(mBidId, Side::BUY, arbitrageBidPrice, std::min((signed int)(LOT_SIZE*bidDiff), POSITION_LIMIT - (int)mPosition), Lifespan::GOOD_FOR_DAY);
            mBids.emplace(mBidId);
        }
        ETFLastAsk = askPrices[0];
        ETFLastBid = bidPrices[0];
    }else if (instrument == Instrument::FUTURE){
        // int askDiff = (ETFLastBid-askPrices[0])/TICK_SIZE_IN_CENTS;
        // int bidDiff = (bidPrices[0]- ETFLastAsk)/TICK_SIZE_IN_CENTS;

        // arbitrageBidPrice = (ETFLastAsk<bidPrices[0]) ? ETFLastAsk - (mPosition / LOT_SIZE) * TICK_SIZE_IN_CENTS : -1;
        ...
        unsigned long bidVol = bidVolumes[0];
        unsigned long askVol = askVolumes[0];
        
        double lambda = (bidVol - askVol)/(bidVol + askVol);

        unsigned long RISK_PARAM = std::abs((long)(askPrices[0]-futureLastBid)/(TICK_SIZE_IN_CENTS));
        unsigned long basePrice = (askPrices[0] + bidPrices[0])/2;
        unsigned long newAskPrice = (askPrices[0] != 0) ? basePrice + (1+RISK_PARAM)*TICK_SIZE_IN_CENTS - (mPosition / LOT_SIZE) * TICK_SIZE_IN_CENTS: 0;
        unsigned long newBidPrice = (bidPrices[0] != 0) ? basePrice - (1+RISK_PARAM)*TICK_SIZE_IN_CENTS - (mPosition / LOT_SIZE) * TICK_SIZE_IN_CENTS: 0;
        if (lambda > 0.5){
            newAskPrice += TICK_SIZE_IN_CENTS;
            newBidPrice += TICK_SIZE_IN_CENTS;
        } else if (lambda < -0.5){
            newAskPrice -= TICK_SIZE_IN_CENTS;
            newBidPrice -= TICK_SIZE_IN_CENTS;
        }
        ...
        if (mAskId == 0 && newAskPrice != 0 && mPosition > -POSITION_LIMIT)
        {
            ...
            SendInsertOrder(mAskId, Side::SELL, newAskPrice, std::min((signed int)(LOT_SIZE), POSITION_LIMIT - (int)mPosition), Lifespan::GOOD_FOR_DAY);
            mAsks.emplace(mAskId);
        }
        if (mBidId == 0 && newBidPrice != 0 && mPosition < POSITION_LIMIT)
        {
            ...
            SendInsertOrder(mBidId, Side::BUY, newBidPrice, std::min((signed int)(LOT_SIZE), POSITION_LIMIT - (int)mPosition), Lifespan::GOOD_FOR_DAY);
            mBids.emplace(mBidId);
        }
    }
}
